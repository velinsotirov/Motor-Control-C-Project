...
void setupADC() {
    ...
    //__HAL_RCC_ADC2_CLK_ENABLE(); not needed for a single shunt!

    // ADC pinout
    // PA0  = ADC0, left shunt
    // PA1  = ADC1, right shunt NO LONGER NEEDED!
    ...
    */
    ...
    adcChannelConfig.SamplingTime = ADC_SAMPLETIME_13CYCLES_5; // with 10.67MHz ADC clock, this gives us 1.265us
    // adc finishes sampling 3.265us after start of PWM period, so we can open the HS 0.5us later, or at 3.765us
    // 2*3.765us/50us = 15% minimum duty cycle!
    // for symmetry, we limit max duty to 85%
